...
Future<void> groupAttachImage({required int id, File? image, bool isCoverImage = false}) async {
  ...
  await sendMultiPartRequest(
    ...
    onSuccess: (data) async {
      ...
      /// todo: add language ley
      toast(isCoverImage ?  'Cover Image Updated Successfully': 'Avatar Image Updated Successfully');
...
Future<CommonMessageResponse> deleteStory({required int storyId}) async {
  Map request = {"story_id": storyId};
...
Future<List<ProductListModel>> getProductsList({int page = 1, int? categoryId, String? orderBy}) async {
  ...
  if (categoryId != null) {
    it = await handleResponse(await buildHttpResponse('${APIEndPoint.productsList}?category=$categoryId&page=$page&per_page=$PER_PAGE', passParameters: true, passToken: false));
  } else {
    it = await handleResponse(await buildHttpResponse('${APIEndPoint.productsList}?orderby=$orderBy&page=$page&per_page=$PER_PAGE', passParameters: true, passToken: false));
  }
...
Future<ProductReviewModel> deleteProductReview({required int reviewId}) async {
  ...
  return ProductReviewModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.productReviews}/$reviewId', request: request, method: HttpMethod.DELETE, passToken: true, passHeaders: false)));
}
...
Future<List<ForumModel>> getForumList({int page = 1}) async {
  Map request = {
    "per_page": PER_PAGE,
    "page": page,
  };
